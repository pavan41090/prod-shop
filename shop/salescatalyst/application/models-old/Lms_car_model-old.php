...
class Lms_car_model extends CI_Model {
   ...
   // var $table = 'tbl_customer';
    ...
    {
        $this->db->select('led.*,cus.*, prdt.product_type, urs.user_location');  
               $this->db->from('tbl_lead AS led');
 //$this->db->join('tbl_users AS usr','usr.channel=led.lead_id');
        $this->db->join('tbl_customer AS cus','led.customer_id = cus.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');
        $this->db->join('tbl_users AS urs','led.created_by = urs.emp_id');
           $i = 0;
        ...
        switch ($this->session->userdata('user_type_abbr')) {
            ...
            case 'report':
                ...
                if(isset($_POST['location']) && $_POST['location']!="" && $_POST['location']!="All"){
                    $this->db->where('urs.user_location',$_POST['location']);
                }
          ...
          if(isset($_POST['startDate']) && isset($_POST['endDate']) && $_POST['startDate']!="" && $_POST['endDate']!="") {
            ...
            case 'supervisor':
                ...
                //$where_clause="Select emp_id from tbl_users WHERE supervisor_id=".$loggedUserId;
...
//rajitha added query
    ...
    {
        $this->db->select('led.lead_id, cus.*,urs.user_location');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cus','led.customer_id = cus.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');
        $this->db->join('tbl_users AS urs','led.created_by = urs.emp_id');
    ...
    public function getLeadDetailsByLeadId($leadId){
        ...
        $this->db->select('led.*, cust.*, prdt.*,prop.*, nomn.*, drhb.*, opt.*');  
        $this->db->from('tbl_lead AS led');
        ...
        $this->db->join('tbl_prop_options AS opt','led.lead_id = opt.lead_id','left');//added left join for nominee details not mantadory
...
//                              prim_status creaded_on  created_by
    ...
    public function getLeadsForExcel($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, cust.*, prdt.*,usr.emp_code,led.created_on as c_on,led.updated_on as u_on');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cust','led.customer_id = cust.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');      
        ...
        if($this->session->userdata('user_type_abbr') == 'hdfc_ops'){ 
            ...
            $this->db->where('led.payment_type', 'Credit Card');
            $this->db->where("(led.lead_status='Sales Closed')", NULL, FALSE);
        } else if( $this->session->userdata('user_type_abbr') == 'bagi_ops' ) {
            if($product !== '' ){ 
            ...
            $this->db->where("(led.lead_status='Ready to Policy' OR led.lead_sub_status='Sales Closed Online')", NULL, FALSE);
        }else if( $this->session->userdata('user_type_abbr') == 'lead data'){
            if(isset($_POST['product']) && $_POST['product']!="" ){
                    $this->db->where('prdt.product_type',$_POST['product']);
                }
                if(isset($_POST['startDate']) && isset($_POST['endDate']) && $_POST['startDate']!="" && $_POST['endDate']!="") {
        ...
        //echo $this->db->last_query();
        //die();
        //var_dump(sizeof($query->result()));
        return $query->result();
    }

    //reports downloads function 

    public function getLeadsForExcelReport($product = NULL, $type = NULL){

        $this->db->select('led.*,led.created_on as c_on,led.updated_on as u_on, cust.*, prdt.*, nomn.*, usr.emp_code,usr.user_location');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cust','led.customer_id = cust.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');      
        $this->db->join('tbl_users AS usr','led.created_by = usr.emp_id'); 
    ...
    public function getLeadsForExcelReportProHome($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*,claim.*, val.*, nomn.* , coms.*,cust.*,prdt.*,  prop.*,usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id supervisorId,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id ,supervisorId) ) supername');  
        $this->db->from('tbl_lead AS led');
 ...
 public function getLeadsForExcelPA($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, cust.*, prdt.*, nomn.*, coms.*,  prop.*, usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id supervisorId,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id ,supervisorId) ) supername');  
        $this->db->from('tbl_lead AS led');
        ...
        $this->db->join('tbl_users AS usr','led.created_by = usr.emp_id'); 
        $this->db->join('tbl_nominee AS nomn','led.lead_id = nomn.lead_id');
        $this->db->join('tbl_comments AS coms', 'led.lead_id =coms.lead_id');
        $this->db->join('tbl_propsal AS prop','led.lead_id = prop.lead_id');
        if(isset($_POST['product']) && $_POST['product']!=""){
                    $this->db->where('prdt.product_type',$_POST['product']);
                }
    ...
    public function getLeadsForExcelReportProSupership($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, cust.*, prdt.*, nomn.*, coms.*,  prop.*, fdr.*, opts.*, usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id supervisorId,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id ,supervisorId) ) supername');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cust','led.customer_id = cust.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');   
        $this->db->join('tbl_users AS usr','led.created_by = usr.emp_id'); 
    ...
    public function getLeadSupervisorFlow($leadId){
        ...
        $this->db->select('chl.enableSupervisor');
        $this->db->from('tbl_lead AS led');
