...
//                              prim_status creaded_on  created_by
    ...
    public function getLeadsForExcel($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, cust.*, prdt.*,usr.emp_code,led.created_on as c_on,led.updated_on as u_on,usr.user_location');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cust','led.customer_id = cust.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');      
    ...
    public function getLeadsForExcelReportProHome($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, led.lead_id leadid, led.created_on createdon, claim.*, val.*, nomn.* , coms.*,cust.*,prdt.*,  prop.*,usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id supervisorId,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id, (select history_user_id from tbl_user_activity_history where history_lead_id = leadid && history_activity_type = 4 ORDER BY history_id DESC LIMIT 1))) supername');  
        $this->db->from('tbl_lead AS led');
 ...
 public function getLeadsForExcelPA($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, led.created_on leadTime,led.created_on createdon, led.lead_id leadid, cust.*, prdt.*, nomn.*, coms.*,  prop.*, usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id supervisorId,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id, (select history_user_id from tbl_user_activity_history where history_lead_id = leadid && history_activity_type = 4 ORDER BY history_id DESC LIMIT 1)) ) supername');  
        $this->db->from('tbl_lead AS led');
    ...
    public function getLeadsForExcelReportProSupership($product = NULL, $type = NULL){
        ...
        $this->db->select('led.*, led.lead_id leadid, led.created_on createdon, cust.*, prdt.*, nomn.*, coms.*,  prop.*, fdr.*, opts.*, usr.emp_code,usr.user_location,usr.user_name,usr.supervisor_id,led.updated_by,(SELECT GROUP_CONCAT(user_name) FROM `tbl_users` WHERE FIND_IN_SET(emp_id, (select history_user_id from tbl_user_activity_history where history_lead_id = leadid && history_activity_type = 4 ORDER BY history_id DESC LIMIT 1))) supername');  
        $this->db->from('tbl_lead AS led');
        $this->db->join('tbl_customer AS cust','led.customer_id = cust.cust_id');
        $this->db->join('tbl_product AS prdt','led.lead_id = prdt.lead_id');   
        $this->db->join('tbl_users AS usr','led.created_by = usr.emp_id'); 
