...
class LmsCar extends CI_Controller {
 	...
 	public function leadListByJson()
    {
    	...
    	switch ($this->session->userdata('user_type_abbr')) {
			...
			case 'bagi_ops':
				...
				$row[] = $lead->payment_type; 
				$row[] = $lead->payment_status;
				//$row[] = '<a href="#">  <button type="button" class="btn blue btn-default downloadReport"  >Download</button></a>';
				$row[] = '';
				break;
			}
            $data[] = $row;
    ...
    private function getUpdateJsonDataLeadStatus(){
    	try{
			...
			if($this->db->affected_rows()>0){
				...
				//'tbl_lead.line_of_business'=>$thproductname
    ...
    public function lmsUpdateProductDetails(){
        ...
        $objPHPExcel->setActiveSheetIndex(0);
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('G1', 'Card Holder Name');
			$objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Relationship with Insured');
			} else {*/
			...
			//$objPHPExcel->getActiveSheet()->SetCellValue('AU1', 'Card Holder Name');
			...
			/*if($this->session->userdata('user_type_abbr')!="hdfc_ops"){
			$objPHPExcel->getActiveSheet()->SetCellValue('AX1', 'Card Holder Name');
        ...
        foreach ($data as $element) {
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $relationDummdata);
			} else {*/
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('AY' . $rowCount, $relationDummdata);
			}*/
            $rowCount++;
            $i++;
      }
     ...
     if($this->session->userdata('user_type_abbr')=="lead data" && ($_POST['product']) == "Combo") {
        ...
        $objPHPExcel->setActiveSheetIndex(0);
            ...
            /*$objPHPExcel->getActiveSheet()->SetCellValue('BI1', 'Card Holder Name');
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('BA' . $rowCount, $element->user_location);
         $objPHPExcel->getActiveSheet()->SetCellValue('BB' . $rowCount, $element->supername);
	...
	elseif($this->session->userdata('user_type_abbr')=="lead data" && ($_POST['product']) == "Personal Accident") {
    	$data = $this->Lms_car_model->getLeadsForExcelPA();
       ...
       $data = $this->Lms_car_model->getLeadsForExcelReportProHome();
        ...
        $objPHPExcel->setActiveSheetIndex(0);
           ...
           /* $objPHPExcel->getActiveSheet()->SetCellValue('CO1', 'Card Holder Name');
      ...
      /*  $objPHPExcel->getActiveSheet()->SetCellValue('BR' . $rowCount, $element->hme_itm_des);
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('CH' . $rowCount, $element->supername);
    ...
    elseif ($this->session->userdata('user_type_abbr')=="lead data" && ($_POST['product']) == "Super Ship"){
        ...
        $objPHPExcel->setActiveSheetIndex(0);
           ...
           /* $objPHPExcel->getActiveSheet()->SetCellValue('CX1', 'Card Holder Name');
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('CT' . $rowCount, $element->supername);
