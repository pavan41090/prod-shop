	                        ...
	                        if(!isset($single_row[0][1])){
	                  		...
	                  		if($leadcurrentstatus !== "Approved"){
								...
								if($quoteData->num_rows()>0){
	                  			...
	                  			$thisStates = $this->db->query("SELECT state_name FROM tbl_master_states INNER JOIN tbl_rta_location ON tbl_rta_location.rta_state = tbl_master_states.state_id WHERE tbl_rta_location.rta_code = '".substr($resulquoteData->reg_number, 1,4)."' LIMIT 1")->row_object();
	                  			print_r($thisStates);
								...
								$twoData['model'] = ($vehicle_details_req->vehiclemodel ? $vehicle_details_req->vehiclemodel : 'DREAMYUGA');
								...
								$twoData['vehicleRegState'] = 'TELANGANA';
								...
								if($letsgetData['StatusCode'] == 200){
									...
									$this->db->set('quote_status',0)->where('quote_lead_sno',$leadlead_id)->update('tbl_vehicle_quote_details');

									if($this->db->affected_rows() > 0){
										$leadQuoteId = $letsgetData['OrderNo'];
										$leadQuoteNo = $letsgetData['QuoteNo'];
										$leadlms_premium = $resulquoteData->lms_premium;
										$responceDatatw = $this->TwoWheeler_model->getCreatepaymentgateWay($leadQuoteId,$leadQuoteNo,$leadlms_premium);
										...
										'quote_orderNum' => $leadQuoteId,
										'quote_QuoteNum' => $leadQuoteNo,
										'lead_policy_number' => ($responceDatatw['PolicyNumber'] ? $responceDatatw['PolicyNumber'] : ''),
										'lead_policy_bagi_transaction_number' => ($responceDatatw['BagiTransactionRefNumber'] ? $responceDatatw['BagiTransactionRefNumber'] : ''),
										'lead_transaction_number' => ( $responceDatatw['TransactionRefNumber'] ? $responceDatatw['TransactionRefNumber'] : ''),
