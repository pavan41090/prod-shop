<?php
    ...
    include_once (dirname(__FILE__) . "/Salescatcontroll.php");

	class Twoexcel extends Salescatcontroll {
    ...
    public function getSearchdataquery(){
        ...
        if(isset($_POST['startDate']) && isset($_POST['endDate']) && $_POST['startDate']!="" && $_POST['endDate']!="") {
            ...
            $this->db->where('motor_added_date >', $startDate->format('Y-m-d'));
            $this->db->where('motor_added_date <', $endDate->format('Y-m-d'));
        }

        $bundledata = $this->db->get('tbl_motor')->result();

            $fileBundleName = 'Motor - '.time().'.xlsx';
            ...
            $objPHPExcel->setActiveSheetIndex(0);
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'SM Name');
                $objPHPExcel->getActiveSheet()->SetCellValue('E1', 'SM Code');
                $objPHPExcel->getActiveSheet()->SetCellValue('F1', 'Customer Name');
                $objPHPExcel->getActiveSheet()->SetCellValue('G1', 'AAN Number');
                $objPHPExcel->getActiveSheet()->SetCellValue('H1', 'Vehicle Type');
                $objPHPExcel->getActiveSheet()->SetCellValue('I1', 'Registration Number');
                $objPHPExcel->getActiveSheet()->SetCellValue('J1', 'City of Registration');
                $objPHPExcel->getActiveSheet()->SetCellValue('K1', 'Policy Expiry Date');
                $objPHPExcel->getActiveSheet()->SetCellValue('L1', 'Previous Year NCB');
                $objPHPExcel->getActiveSheet()->SetCellValue('M1', 'Previous Insurance Company');
                ...
                foreach ($bundledata as $element) {
                    ...
                    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $element->motor_category);
                    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $element->motor_employee_code);
                    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $element->motor_sm_name);
                    $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $element->motor_sm_code);
                    $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $element->motor_customer_name);
                    $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $element->motor_aaa_number);
                    $objPHPExcel->getActiveSheet()->SetCellValue('H' . $rowCount, $element->motor_vehicle_type);
                    $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $element->motor_register_number);
                    $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $element->motor_city_registration);
                    $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $element->motor_expire_date);
                    $objPHPExcel->getActiveSheet()->SetCellValue('L' . $rowCount, $element->motor_previous_ncb);
                    $objPHPExcel->getActiveSheet()->SetCellValue('M' . $rowCount, $element->motor_previous_company);
    ...
    private function _getSearchBundle(){
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST"){
            ...
            $objPHPExcel->setActiveSheetIndex(0);
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('AE1', 'Bundle PA Premium');
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('BQ1', 'Child 333 Name');
                //UPP LEAD
                $objPHPExcel->getActiveSheet()->SetCellValue('BR1', 'UPP Loan Type');
                $objPHPExcel->getActiveSheet()->SetCellValue('BS1', 'UPP Loan Amount');
                $objPHPExcel->getActiveSheet()->SetCellValue('BT1', 'UPP Loan Tenure');
                $objPHPExcel->getActiveSheet()->SetCellValue('BU1', 'UPP PA Sum Insured');
                $objPHPExcel->getActiveSheet()->SetCellValue('BV1', 'UPP Accidental Hospitalization Sum Insured');
                $objPHPExcel->getActiveSheet()->SetCellValue('BW1', 'UPP Credit Shield Sum insured');
                $objPHPExcel->getActiveSheet()->SetCellValue('BX1', 'UPP Critical Illness Sum insured');
                $objPHPExcel->getActiveSheet()->SetCellValue('BY1', 'UPP Long Term Premium');
                $objPHPExcel->getActiveSheet()->SetCellValue('Bz1', 'Total Premium');
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('AE' . $rowCount, $bundleData->bundle_premium_amount);
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('BR' . $rowCount, ($bundleData->bundle_include_upp != '0'? ($bundleData->bundle_upp_type_of_loan == '1' ? 'Personal' : 'Business') :''));



            $objPHPExcel->getActiveSheet()->SetCellValue('BS' . $rowCount, ($bundleData->bundle_include_upp == '0' ? '' : $bundleData->bundle_upp_loan_amount));
            //$objPHPExcel->getActiveSheet()->SetCellValue('BS' . $rowCount, $bundleData->bundle_upp_loan_amount);
            $objPHPExcel->getActiveSheet()->SetCellValue('BT' . $rowCount, ($bundleData->bundle_include_upp == '0' ? '' : $tenureDat[$bundleData->bundle_upp_tenure]));
            $objPHPExcel->getActiveSheet()->SetCellValue('BU' . $rowCount, $bundleData->bundle_pa_sum);
            $objPHPExcel->getActiveSheet()->SetCellValue('BV' . $rowCount, $bundleData->bundle_hospi);
            $objPHPExcel->getActiveSheet()->SetCellValue('BW' . $rowCount, $bundleData->bundle_credit);
            $objPHPExcel->getActiveSheet()->SetCellValue('BX' . $rowCount, $bundleData->bundle_critical);
            $objPHPExcel->getActiveSheet()->SetCellValue('BY' . $rowCount, $bundleData->bundle_upp_total_prem);
            $objPHPExcel->getActiveSheet()->SetCellValue('BZ' . $rowCount, $bundleData->total_premium == '0' ? $element->lms_premium : $bundleData->total_premium);
    ...
    public function getSearchMotor(){
