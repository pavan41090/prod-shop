...
*/
	...
	public function logout() {
		$data['logOutTime'] = date('Y-m-d G:i:s');
		$updateHist = $this->Useractivity->getInsertHistory(array(
					'emp_id' => $this->session->userdata('emp_id'),
					'leadId' => 0,
					'type' => 10, //lead status changing,
	...
	private function getDownloadUsersSupervior($supervisorone,$supervisortwo,$supervisorthree,$query){
		try{

			foreach ($query as $key => $value) {

				# code...
				if(!in_array( $value->emp_id, $supervisorone )){
	        		
	        		if(!in_array($value->emp_id, $supervisortwo)){

	        			if(!in_array($value->emp_id, $supervisorthree)){
	...
	public function getAllSupervisor(){
        ...
        $supervisorthree='';
        foreach ($query as $key => $value) {
        	$supervisorId = explode(',',$value->supervisor_id);

        	if($supervisorId[0] || empty($supervisorId[0])){
        		$supervisorone .= $supervisorId[0].',';
        	}
        		$supervisortwo .= $supervisorId[1].',';
        	
        		$supervisorthree .= $supervisorId[2].',';
        	
        }
       $trimone = trim($supervisorone,',');
       $trimtwo = trim($supervisortwo,',');
       $trimthree = trim($supervisorthree,',');
       $arraysupervisorone = explode(',',$trimone);        
       $arraysupervisortwo = explode(',',$trimtwo);        
       $arraysupervisorthree = explode(',', $trimthree);        
       return $this->getDownloadUsersSupervior($arraysupervisorone,$arraysupervisortwo,$arraysupervisorthree,$query);
