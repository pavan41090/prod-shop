...
class LmsCar extends Twoexcel {
		...
		if ($this->session->userdata('logged_in') == TRUE) {
			...
			if(count($userRight) == 0){
				...
				if($userRight[0]->prdt_m_shot_name !== 'car'){
					redirect('lms-'.$userRight[0]->prdt_m_shot_name, 'refresh');
					exit();
				} else {
					$this->load->view('layout/header_inner');
					$this->load->view('layout/menu_inner',$this->data);
    ...
    public function leadViewByLeadId(){
		...
		#print_r($this->data['lead_details']);
#echo 'string';
		$this->load->view('layout/header_inner');
		$this->load->view('layout/menu_inner',$this->data);

#echo $this->data['lead_details'][0]->product_type;
#die;
...
case 'UPP LONG TERM':
    ...
    public function lmsUpdateProductDetails(){
        ...
        $objPHPExcel->setActiveSheetIndex(0);
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('V1', 'Business_Type');
...
if($_POST['product'] == "UPP Long Term"){
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('W' . $rowCount, $upp_all_json_details->peraccpremium);
			$objPHPExcel->getActiveSheet()->SetCellValue('X' . $rowCount, $upp_all_json_details->finaltotalpremium);//SumInsured
			} else {
			$objPHPExcel->getActiveSheet()->SetCellValue('W' . $rowCount, $element->lms_premium);
			$objPHPExcel->getActiveSheet()->SetCellValue('X' . $rowCount, $element->sum_insured);//SumInsured
			}
			...
			$qrytblpolicydetailstable = $this->db->select('policy_number,policy_start_date,policy_end_date')->where(array('policy_lead_no'=>$element->lead_id))->order_by('policy_sno','DESC')->limit(1)->get('tbl_policy_details_table');
			$tblpolicydetailstable = $qrytblpolicydetailstable->row_object();
			if($qrytblpolicydetailstable->num_rows() == 0){

				//($tblpolicydetailstable->policy_number ? $tblpolicydetailstable->policy_number : '')
			$objPHPExcel->getActiveSheet()->SetCellValue('AU' . $rowCount, '');
			//($tblpolicydetailstable->policy_start_date ? $tblpolicydetailstable->policy_start_date : '')
			$objPHPExcel->getActiveSheet()->SetCellValue('AV' . $rowCount, '');
			//$tblpolicydetailstable->policy_end_date ? $tblpolicydetailstable->policy_end_date : '')
			$objPHPExcel->getActiveSheet()->SetCellValue('AW' . $rowCount, '');

			} else {

//
			$objPHPExcel->getActiveSheet()->SetCellValue('AU' . $rowCount, ($tblpolicydetailstable->policy_number ? $tblpolicydetailstable->policy_number : ''));
			//
			$objPHPExcel->getActiveSheet()->SetCellValue('AV' . $rowCount, ($tblpolicydetailstable->policy_start_date ? $tblpolicydetailstable->policy_start_date : ''));
			//
			$objPHPExcel->getActiveSheet()->SetCellValue('AW' . $rowCount, ($tblpolicydetailstable->policy_end_date ? $tblpolicydetailstable->policy_end_date : ''));
...
if($this->session->userdata('user_type_abbr')=="lead data" && ($_POST['product']) == "UPP Long Term") {
			...
			$objPHPExcel->getActiveSheet()->SetCellValue('BS' . $rowCount, $upp_all_json_details->finaltotalpremium);
            $rowCount++;
            $i++;
      }
     ...
     else if($this->session->userdata('user_type_abbr')=="lead data" && ($_POST['product']) == "Combo") {
...
elseif ($this->session->userdata('user_type_abbr')=="report" && ($_POST['product']) == "Motor"){
    ...
    /********** download report save time zone ****************/
       ...
       // header("Content-type: application/octet-stream");
		...
		//elseif($this->session->userdata('user_type_abbr')=="report"){
		//exit();   
		header("Content-Type: application/vnd.ms-excel");
	   header('Content-Disposition: attachment;filename="'.$fileName.'"');
	   header("Content-Transfer-Encoding: binary ");
