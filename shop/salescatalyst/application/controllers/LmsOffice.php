...
ob_start();
class LmsOffice extends CI_Controller {
	...
	public function createLmsOffice(){

		if ($this->session->userdata('logged_in') == TRUE) {

			$this->data['module'] = 'leads';
			$this->data['sub_module'] = 'Office'; 
			...
			$this->load->view('lms_view/lms_create_office',$this->data);
	...
	public function lmsOfficeProductDetails(){
		...
		$productData = array(
			...
			'expiring_policy_NCB'  => '',
            'hme_building_type'=> $this->input->get_post('office_building_type'),
            'hme_property_ownership'=> $this->input->get_post('office_property_ownership'),
            'hme_property_type'=> '',
            'hme_previous_claims'=> $this->input->get_post('office_previous_claims'),
            'hme_no_of_floors'=> $this->input->get_post('office_no_of_floors'),
            'hme_year_of_construction'=> $this->input->get_post('office_year_of_construction'),
            'hme_independent_house'=> '',
            'hme_compound_wall'=> '',
            'hme_build_up'=> '',
            'hme_sum_insured_provided'=> $this->input->get_post('office_sum_insured_provided'),
            'hme_risk_address_same'=> $this->input->get_post('office_risk_address_same'),
            'hme_risk_address1'=> $this->input->get_post('office_risk_address1'),
            'hme_risk_address2'=> $this->input->get_post('office_risk_address2'),
            'hme_risk_area' => $this->input->get_post('office_risk_area'),
            'hme_risk_pincode'=> $this->input->get_post('office_risk_pincode'),
            'hme_risk_state'=> $this->input->get_post('office_risk_state'),
            'hme_risk_city'=> $this->input->get_post('office_risk_city'),
            'hme_risk_nearest_land_mark'=> $this->input->get_post('office_risk_nearest_land_mark'),

			'sum_insured' => $this->input->get_post('office_sum_insured'),
		...
		for($i = 1; $i<5; $i++ ){
			if($this->input->get_post('office_long_des'.$i) != ''){
				$claimData = array(
					'customer_id'  		=> $this->session->userdata('customerId'),
					'lead_id' 	   		=> $this->session->userdata('leadId'),
					'hme_long_des'		=> $this->input->get_post('office_long_des'.$i),
					'hme_assets_damage'	=> $this->input->get_post('office_assets_damage'.$i),
					'hme_cause_loss'	=> $this->input->get_post('office_cause_loss'.$i),
					'hme_ins_place'		=> $this->input->get_post('office_ins_place'.$i),
					'hme_policy_yr'		=> $this->input->get_post('office_policy_yr'.$i),
					'hme_ins_claim'		=> $this->input->get_post('office_ins_claim'.$i),
					'hme_loss_amt'		=> $this->input->get_post('office_loss_amt'.$i),
					'status'			=> '1',
					'created_by'		=> $this->session->userdata('emp_id'),	
				);
	...
	public function lmsOfficeProposalDetails(){
		...
		$proposalData = array(
			...
			'new_policy_start' 		=> $this->input->get_post('office_pro_policy_sdate'),
			'prop_mtr_reg_date' 	=> '',
			'prop_mtr_regi_num' 	=> '',
			'prop_mtr_engine_num' 	=> '',
			'prop_mtr_chasis_num'	=> '',
		...
		$nomineeData = array(
			'customer_id'  	 	=>$this->session->userdata('customerId'),
			'lead_id' 	   	    =>$this->session->userdata('leadId'),
			'nominee_name'		=> $this->input->get_post('office_pro_nomie_name'),
			'nominee_age' 		=> $this->input->get_post('office_pro_nomie_age'),
			'nominee_relationship'	=> $this->input->get_post('office_pro_nomie_relation'),
			'appointee_name' 		=> $this->input->get_post('office_pro_nameofappoint'),
			'appointee_relationship'	=> $this->input->get_post('office_pro_appoint_relation'),
			'created_by'			=> $this->session->userdata('emp_id'),
...
public function lmsOfficeScriptQuery()
  {
	...
	public function lmsOfficeUpdateProductDetails(){
$statusData = array(
			'lead_status'=>"Lead Generated",
