...
public function __construct(){
    parent::__construct();
    $this->column_search = array('tbl_lead.lead_application_id','tbl_lead.first_name','tbl_lead.last_name','tbl_lead.cust_phone','tbl_lead.cust_city','tbl_lead.lead_status');
}
...
private function __bagigetleadsDashboard($page,$limit){
    ...
    $searhleaddata = $thisPost['searhleaddata'];
    ...
    if($this->session->userdata('user_type_abbr') == 'bagi_av'){
        $this->db->where('(tbl_lead.lead_status = "Lead Generated" OR tbl_lead.lead_status = "Follow Up" OR tbl_lead.lead_status = "Not Contacted")');
    ...
    if($this->session->userdata('user_type_abbr') == 'supervisor'){
        ...
        $this->db->where('(tbl_lead.lead_status = "Sales Closed" OR tbl_lead.lead_status = "Sales Closed Online")');
    ...
    foreach ($this->column_search as $item){
        if($searhleaddata){
            if($i===0){
                $this->db->group_start(); 
                $this->db->like($item, $searhleaddata);
            } else {
                $this->db->or_like($item, $searhleaddata);
            }
    ...
    $i= ($this->uri->segment(2) > 1 ? $this->uri->segment(2) : 0)+1;
    if($numberRows>0){
    foreach ($resultingData->result() as $key => $value) {
        # code...
        $tableViewhtml = '';
        $detailLink = base_url().'view-lead-details/'.$value->lead_id;
        $editLink = base_url().'regenerate-lead/'.$value->lead_id;
        if($value->lead_status == 'Sales Open'){
        $tableViewhtml = '<a href="'.$editLink.'"><button class="btn blue btn-default">View / Edit</button></a>';
        } else {
        $tableViewhtml = '<a href="'.$detailLink.'"><button class="btn blue btn-default">View</button></a>';
        }
        $tableDisplayhtml .= '<tr>
        <td>'.$i.'</td>
        <td>'.$value->lead_application_id.'</td>
        ...
        if($value->lead_status == 'Sales Open'){
        $tableDisplayhtml .= '<th><a href="'.$editLink.'"><button class="btn blue btn-default">View / Edit</button></a></th>';
        } else {
        $tableDisplayhtml .= '<th><a href="'.$detailLink.'"><button class="btn blue btn-default">View</button></a></th>';
        }
        $tableDisplayhtml .= '</tr>';

    $i++; }
    } else {
        $tableDisplayhtml .= '<tr style="text-align:center;"><td colspan="9">No Records Found</td></tr>';
    }
    $tableDisplayhtml .= '<tr style="text-align:right;"><td colspan="9">Total Count:'.$numberRows.'</td></tr>';
    $tableDisplayhtml .= '</thead>';
    $returnDatadid['numrowsData'] = $tableDisplayhtml;
    #echo json_encode($output);
