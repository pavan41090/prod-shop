			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if( $i < $highestRow){
                	...
                	if(!empty($leadNumber)){
                		...
                		$dataInsert['policy_lead_no'] = '(SELECT IFNULL(lead_id,0) lead_id FROM '.TABLE_LEAD.' WHERE lead_application_id = "'.$leadNumber.'" )';
                        $AdataInsert['policy_number'] = $leadpolicynumber;
	                	$AdataInsert['policy_start_date'] = $pstartDate; 
	                	$AdataInsert['policy_end_date'] = $pendDate;

                        $AdataInsert['policy_number_2'] = $secondPolDate;
                        $AdataInsert['policy_start_date_2'] = $secondpStart;
                        $AdataInsert['policy_end_date_2'] = $secondSendDate;
