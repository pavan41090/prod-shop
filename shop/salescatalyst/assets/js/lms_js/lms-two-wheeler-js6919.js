			...
			$scope.ncbCheckerU = function(selectedVaue){
				...
				$http({
						...
						for (var i=0; i < checkValuencb.length; i++) {
							optionTextncb += "<option value = '"+checkValuencb[i]+"'>"+checkValuencb[i]+"</option>";
						}
...
console.log(_datainfo._dom);
						...
						if(_datainfo._fststatus == true){
							...
							/////$scope.xncbCheckerU( _datainfo._dom );


				var vehicleObj = new Object();
				vehicleObj.value = _datainfo._dom;
				$http({
						url : __pathname+'get-man-fac',
						method : 'POST',
						cache : false,
			...
			$scope.getQuoteInformation = function(){
				...
				try{
					...
					var webUrl = angular.element(document.getElementById('web_url')).val();
					var vehicleregDate = angular.element(document.getElementById('lms_two_wheeler_registration_date')).val();
					var vehicleManDate = angular.element(document.getElementById('lms_two_wheeler_man_year')).val();
					var pypstartsdate = angular.element(document.getElementById('lsm_car_pyp_starts_date')).val() || '';
					var pypendsdate = angular.element(document.getElementById('lsm_car_pyp_ends_date')).val() || '';
					var lmscartenure = angular.element(document.getElementById('lms_car_tenure')).val() || '0';
					...
					var vehicleRegDa = vehicleRegisterDate || vehicleregDate;
...
if(quiteData.StatusCode != 200){
			...
			try{
				...
				_vehiclerikTypeObject.value = vehiclerikType || $('#vehiclerikType').val();
				...
				_vehiclefueltypeObject.value = vehiclefueltype || $('#vehiclefueltype').val();
				_twowheelArray.push(_vehiclefueltypeObject);
				var _vehicleVariantObject = new Object();
				_vehicleVariantObject.name = 'vehicleVariant';
				_vehicleVariantObject.value = vehicleVariant || $('#vehicleVariant').val();
				...
				_vehicleccObject.value = vehiclecc || $('#vehiclecc').val();
				_twowheelArray.push(_vehicleccObject);
				var _nextObject = new Object();
				_nextObject.name = 'vehicleSeatingCapacity';
				_nextObject.value = vehicleSeatingCapacity || $('#vehicleSeatingCapacity').val();
				...
				$http({
						...
						if(_statusreport.addon == true){
                    		...
                    		premioumTable += '<tr><td>NCB</td><td>:</td><td>'+Fpremiumcover[0].ExtraDetails.BreakUp.NCB+'</td></tr>';
