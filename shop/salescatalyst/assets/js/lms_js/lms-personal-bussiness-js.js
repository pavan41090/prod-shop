...
return {
	...
	controller:function($scope,$http){
		...
		$scope.calculateagedob = function(){
				...
				if(years < 18  && years >= 0){
				$('#error-age-message').html('').append('Minimum Age of Insured should be 18 years');
				} else if( years <= 60 && years >= 18 ) {
				$('#error-age-message').html('');
				return true;
				} else {
				$('#error-age-message').html('').append('Outside Max Age id 18-60 years');
				return false;
				}
