...
class Shop extends MX_Controller {
	...
	private function callApiShop(){
		$this->Shopurlmodel->urlPost();
	}

	public function turtlemint(){
		$this->callApiShop();
	}
	
	public function getResponce(){
		
		$leadDataReport = $this->db->where('shop_cus_id',$this->uri->segment(2))->get('tbl_shop_office')->row_object();
		$thisInsert = "INSERT INTO tbl_shop_bill_res SET shop_id = IFNULL((SELECT shop_sno FROM tbl_shop_office WHERE shop_cus_id = '".$this->uri->segment(2)."'),0),shop_bill_res = '".json_encode($_POST)."' ";
		
		$this->db->query($thisInsert);
		if($this->db->affected_rows() > 0){
			$leadOrderNumber = $leadDataReport->lead_order_id;
			$this->Shopmodel->senMailCmpleteOrderID($leadOrderNumber,$this->uri->segment(2));
		}
		$dataPlanName['information'] = $leadDataReport;
		$this->load->view('payment-success',$dataPlanName);
	}
	
	public function getReloadResponce(){
	...
	public function getinfoThird(){
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			$this->session->set_userdata('sessionPlanname',$shopPlanName,true);
	...
	public function index()
	{
		...
		try{
			...
			if($_SERVER['REQUEST_METHOD'] == "POST"){
				...
				$this->session->set_userdata('sessionPlanname','5',true);
				$this->load->view('index',$dataPlanName);
		...
		try{
			...
			if($this->db->affected_rows()>0){
				...
				if($sttausMessage && $affectedRows){
					...
					$otpStatusRes['status'] = true;
					$otpStatusRes['message'] = "Something went wrong 1! Please try after some time!";
				}
	...
	public function getSubmitOtp(){
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			$selectOtp = $this->db->select('shop_status,shop_sno,shop_otp_customer')->where(
				array(
				...
				if($rowselectOtp->shop_status == 0){
					...
					if($rowselectOtp->shop_otp_customer == $resinputOtp){
						...
						if($this->db->affected_rows()>0){
							...
							$leadOrderNumber = 'BAGI'.date('Ymd').$rowselectOtp->shop_sno;//$this->Shopmodel->generateApplicationNumber();
   ...
   public function getCityByPincode(){
	...
	public function getCreatepayment(){
		...
		$nnmaeplan = $this->session->userdata('sessionPlanname');
			if($nnmaeplan == '5'){
              $plannameDis = 876;
			...
			$leadDataReport = $this->db->where('shop_cus_id',$this->uri->segment(2))->get('tbl_shop_office')->row_object();
			$planAmount['information'] = $leadDataReport;
			$this->load->view('payment-process',$planAmount);
	}
